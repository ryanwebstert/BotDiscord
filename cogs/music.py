...
class music(commands.Cog):
    ...
    async def p(self, ctx, *args):
        ...
        try:
            ...
            embedvc = discord.Embed(
                colour= 1646116,#grey
                description = 'Para tocar a música você precisa estar em um canal de voz seu imbecil'
            )
            ...
            if type(song) == type(True):
                embedvc = discord.Embed(
                    colour= 12255232,#red
                    description = 'Deu merda! Escreve direito macaco'
                )
                await ctx.send(embed=embedvc)
            else:
                embedvc = discord.Embed(
                    colour= 32768,#green
                    description = f"Você adicionou essa música de merda **{song['title']}** \n"
                )
                await ctx.send(embed=embedvc)
    ...
    async def q(self, ctx):
        ...
        if retval != "":
            ...
            embedvc = discord.Embed(
                colour= 1646116,
                description = 'Não existe músicas na fila, imbecil.'
            )
            await ctx.send(embed=embedvc)
    ...
    async def skip(self, ctx):
        if self.vc != "" and self.vc:
            ...
            embedvc = discord.Embed(
                ...
                description = f"Você pulou a música, lixo."
            )
            await ctx.send(embed=embedvc)
